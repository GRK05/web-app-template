...
resource "google_compute_url_map" "default" {
  ...
  path_matcher {
    ...
    dynamic "path_rule" {
      ...
      content {
        paths   = ["/${path_rule.key}"]
        service = google_compute_backend_service.dynamic_services[path_rule.key].id
      }
    }
  }
}
...
resource "google_compute_backend_service" "default" {
  ...
  dynamic "backend" {
    for_each = toset(var.url_map_default)
    content {
...
resource "google_compute_backend_service" "dynamic_services" {
  provider = google-beta
  for_each = var.url_map
